...
MeshData* GeometryFactory::LoadOrGetMeshFromFile(std::wstring path, bool invertV)
{
	...
	std::wstring finalPath = GEOMETRIES_FOLDER + path;
	meshFile.open(finalPath.c_str(), std::ios::in);
	
	std::vector<glm::vec2> tempTexCoords;
	std::vector<glm::vec3> tempPosition;
	...
	for(std::string line; std::getline(meshFile, line); )   // Read stream line by line
	{
	    ...
	    { 
	        float u, v;
	        in >> u >> v;
	        tempTexCoords.push_back( glm::vec2(u, invertV ? 1 -v : v) ); // Flip V to match DirectX/OpenGL convention
	    } 
	    else if (type == "f") 
	    {
