...
Editor::Editor(GuiHandler* guiHandler)
{
    ...
    m_renderScene = new RenderTarget(format, 1920, 1080);
    m_renderScene->setRenderContext(&m_renderWindow->getRenderContext());
    
    m_defaultTexture = new Texture(*m_renderScene, "sunflower.jpg");
    m_defaultSampler = new Sampler();
    
    m_renderPipeline = new RenderPipeline(*m_defaultTexture, *m_defaultSampler, *m_renderScene, { &sFragment, &sVertex});
    
    m_nodeEditor = new NodeEditor(guiHandler);
    m_guiHandler = guiHandler;

    m_mesh = new Mesh(m_renderWindow->getRenderContext(), GeometryFactory::GetPrimitive(Primitive::CUBE));
...
void Editor::draw()
{
    ...
    m_renderScene->beginDraw();
    
    m_renderScene->drawObject(*m_renderPipeline, *m_testObject);
    
    m_renderScene->endDraw();
    

    m_renderWindow->beginDraw();
    ...
    
    {
        ...
        m_renderPipeline->Update(m_renderWindow->GetGlobalBuffer(), m_renderWindow->getRenderContext().getCurrentFrame());
    ...
    ImGui_ImplVulkan_RenderDrawData(draw_data, m_renderWindow->getRenderContext().getCommandBuffer());
    
    m_renderWindow->endDraw();
...
bool Editor::isEditorClosed()
{
    return m_renderWindow->shouldClose();
}
